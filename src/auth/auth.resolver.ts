...
export class AuthResolver {
  ...
  async register(@Args('newUserData') newUserData: NewUserInput): Promise<UserWithToken> {
    return this.commandBus.execute(await new RegisterUserCommand(newUserData))
  }

  @Mutation(() => UserWithToken)
  async login(@Args('loginUserData') loginUserData: LoginUserInput): Promise<UserWithToken> {
